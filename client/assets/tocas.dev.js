    ...
    return ts(this).find("thead th").each(function(i) {
      return ts(this).on("click", function() {
        ...
        sortTable = function(table, col, reverse) {
          var element, j, len, results, tb, tr;
          tb = table.tBodies[0];
          ...
          results = [];
          for (j = 0, len = tr.length; j < len; j++) {
            element = tr[j];
            results.push(tb.appendChild(element));
          }
          return results;
        };
        sortTable(table, i, isAsc);
        return ts(this).addClass(isAsc ? 'sorted descending' : 'sorted ascending');
      });
    });
  });
};
...
ts.fn.embed = function(option) {
  return this.each(function() {
    ...
    embedEl = this;
    if (this.getAttribute('data-embed-actived')) {
      return;
    }
    if (query !== '') {
      query = '?' + query;
